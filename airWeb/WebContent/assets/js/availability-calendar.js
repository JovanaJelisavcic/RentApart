(function ($) {
	...
	AvailabilityCalendar.prototype = {
		...
		__createToolbar: function () {
			...
			this.$monthLabel = $('<span id="monthLabel"></span>').appendTo($toolbar);
		...
		__createTable: function () {
			var $table = $('<table id="calendarTable"></table>').appendTo(this.container);
		...
		__addEvents: function (cellIndexes, dates) {
			...
			this.bookedDates.forEach(function (date) {
				if (date.begin <= lastDate && date.end >= firstDate) {
					var startIndex = cellIndexes[date.begin];
